...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Append to dump/plot filenames (e.g. c0 for parallel chunks).",
    )
...
def main() -> None:
    ...
    tag_suffix = f"_{args.output_suffix}" if args.output_suffix else ""
    ...
    if fig is not None:
        fig.savefig(
            plot_dir
            / f"comparison_fthmc_L{args.lattice_size}_beta{beta_tag}_nsteps{args.n_steps}_{args.save_tag}{tag_suffix}.pdf",
            transparent=True,
        )

    np.savetxt(
        dump_dir
        / f"topo_fthmc_L{args.lattice_size}_beta{beta_tag}_nsteps{args.n_steps}_{args.save_tag}{tag_suffix}.csv",
        np.array(topo),
        fmt="%.6e",
    )
    np.savetxt(
        dump_dir
        / f"accept_rate_fthmc_L{args.lattice_size}_beta{beta_tag}_nsteps{args.n_steps}_{args.save_tag}{tag_suffix}.csv",
        [acceptance_rate],
        fmt="%.6e",
    )
