...
The U(2) implementation internally stores links as a U(1) phase plus an SU(2) unit quaternion and exports generated configs as complex `2x2` matrices. Training converts those matrix configs back to the split representation on load. The base U(2) field transformation uses plaquette and rectangle loop terms projected into U(2) trace/traceless sin-like and cos-like components, updates full U(2) links with four Lie-algebra coefficients per active link, and evaluates the non-volume-preserving Jacobian as exact `4x4` active-link tangent blocks. Checkpoints from the earlier volume-preserving U(2) base model are not compatible with this transform. U(2) training exposes `--if_check_jac` for small manual-vs-autograd Jacobian checks and `--if_compile` for optional `torch.compile` wrappers.
