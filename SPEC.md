...
The U(2) implementation internally stores links as a U(1) phase plus an SU(2) unit quaternion and exports generated configs as complex `2x2` matrices. Training converts those matrix configs back to the split representation on load. The base U(2) field transformation feeds gauge-invariant scalar plaquette and rectangle loop features to the CNN, keeps the learned coefficient layout compatible with four Lie-algebra slots per loop, and currently applies only the central phase-update slots while the traceless slots remain zero. It evaluates the non-volume-preserving Jacobian as exact `4x4` active-link tangent blocks. Checkpoints from earlier U(2) base model definitions are not compatible with this transform. U(2) training exposes `--if_check_jac` for small manual-vs-autograd Jacobian checks; `--if_compile` is optional and cannot be combined with Jacobian checking. FT-HMC evaluation exposes an explicit evaluation-only compile path that freezes models and uses compiled callables only for force calculations.
